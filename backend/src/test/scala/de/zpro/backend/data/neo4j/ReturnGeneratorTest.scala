...
class ReturnGeneratorTest extends AnyFlatSpec with should.Matchers {
  "The ReturnGenerator" should "return a simple Node" in {
    ...
    response should be("WITH name\r\nRETURN name.prop1 AS prop1, name.prop2 AS prop2, name.prop3 AS prop3")
  }
  ...
  it should "return a dogs litter" in {
    ...
    val response = ReturnGenerator.generateReturnString(request)
    response should be("WITH d, {prop1: l.prop1, prop2: l.prop2} AS l\r\nRETURN d.prop3 AS prop3, d.prop4 AS prop4, COLLECT(l) AS l")
  }
