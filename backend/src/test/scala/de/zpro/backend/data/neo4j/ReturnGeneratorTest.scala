...
class ReturnGeneratorTest extends AnyFlatSpec with should.Matchers {
  "The ReturnGenerator" should "return a simple Node" in {
    ...
    response should be("WITH {prop1: name.prop1, prop2: name.prop2, prop3: name.prop3} AS name\r\nRETURN name")
  }
  ...
  it should "return a dogs litter" in {
    ...
    val response = ReturnGenerator.generateReturnString(request)
    response should be("WITH {prop3: d.prop3, prop4: d.prop4} AS d, {prop1: l.prop1, prop2: l.prop2} AS l\r\nRETURN d, COLLECT(l) AS l")
  }
  ...
  it should "return a dogs parents through its litter" in {
    ...
    val response = ReturnGenerator.generateReturnString(request)
    response should be("WITH {prop7: d.prop7, prop8: d.prop8} AS d, {prop5: l.prop5, prop6: l.prop6} AS l, {prop1: m.prop1, prop2: m.prop2} AS m, {prop3: f.prop3, prop4: f.prop4} AS f\r\n" +
      "RETURN d, COLLECT(l) AS l, COLLECT(m) AS m, COLLECT(f) AS f")
  }
